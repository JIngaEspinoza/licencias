...
export default function Sidebar({ open, onClose }: SidebarProps) {
  ...
  //const esEdificaciones = user?.roles?.includes("EDIFICACIONES");
    ...
    <>
      ...
      `}>
        ...
        <nav className="p-4 space-y-2 overflow-y-auto h-[calc(100vh-160px)]">
          ...
          {/* Admin Section */}
          {auth.current()?.roles?.includes("ADMIN") && (
            <div className="pt-4">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-black/10 border-t border-white/10">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-white flex items-center justify-center text-xs font-bold text-[#0c7286] shrink-0">
              {auth.current()?.email?.substring(0,2).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0 text-white">
              <p className="text-xs font-bold truncate">{auth.current()?.email}</p>
              <p className="text-[10px] text-white/60 truncate capitalize">{auth.current()?.roles?.join(", ")}</p>
            </div>
          </div>
        </div>
      </aside>
